  ...
  <div class="row" style="margin-top:10px;">
  	...
  	<div class="col-sm-10" style="background-color: #fff; padding-top: 25px;">
		...
		if(!empty($_POST['komunitas_cari'])){
	  		$komunitas_nama = $_POST['komunitas_cari'];
	  		$query = mysqli_query($konek, "SELECT *, komunitas.komunitas_id AS kom_id FROM komunitas LEFT JOIN anggota_komunitas ON komunitas.komunitas_id=anggota_komunitas.komunitas_id WHERE komunitas_nama LIKE '%$komunitas_nama%' AND (anggota_id='$anggota_id' OR anggota_id IS NULL) limit $posisi,$batas");	
	  	}else{
		...
		if(!$query){
  		...
  		<div class="col-sm-6 box-kegiatan" style="margin-top:10px;">
  			...
  			<div class="col-sm-7" style="margin-top:-15px;">
  				<div class="row" style="min-height: 150px;">
	  				...
	  				Deskripsi Komunitas:<?php potong_caption($row['komunitas_desk'],90) ?>
	  			</div>
		  ...
		  $paging2 = mysqli_query($konek,"SELECT * FROM komunitas");
